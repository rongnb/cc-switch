...
pub(crate) fn write_live_snapshot(app_type: &AppType, provider: &Provider) -> Result<(), AppError> {
    match app_type {
        ...
        AppType::Hermes => {
            // Hermes uses additive mode - write provider to config
            use crate::hermes_config;
            use crate::hermes_config::HermesProviderConfig;

            // Convert settings_config to HermesProviderConfig
            let hermes_config_result =
                serde_json::from_value::<HermesProviderConfig>(provider.settings_config.clone());

            match hermes_config_result {
                Ok(config) => {
                    hermes_config::set_typed_provider(&provider.id, &config)?;
                    log::info!("Hermes provider '{}' written to live config", provider.id);
                }
                Err(e) => {
                    log::warn!(
                        "Failed to parse Hermes provider config for '{}': {}",
                        provider.id,
                        e
                    );
                    // Try to write as raw JSON if it looks valid
                        ...
                        || provider.settings_config.get("models").is_some()
                    {
                        ...
                        log::info!(
                            "Hermes provider '{}' written as raw JSON to live config",
                            provider.id
                        );
                    } else {
                        return Err(AppError::Message(format!(
                            "Hermes provider '{}' has invalid config structure for live config (must contain 'baseUrl', 'api', or 'models')",
                            provider.id
                        )));
                    }
                }
...
pub fn import_default_config(state: &AppState, app_type: AppType) -> Result<bool, AppError> {
    // Additive mode apps (OpenCode, OpenClaw, Hermes) should use their dedicated
...
///
/// This imports existing providers from ~/.hermes/hermes.json
/// into the CC Switch database. Each provider found will be added to the
/// database with is_current set to false.
pub fn import_hermes_providers_from_live(state: &AppState) -> Result<usize, AppError> {
    use crate::hermes_config;

    let providers = hermes_config::get_typed_providers()?;
    if providers.is_empty() {
        return Ok(0);
    }
    ...
    for (id, config) in providers {
        ...
        if config.models.is_empty() {
            log::warn!("Skipping Hermes provider '{id}': no models defined");
            continue;
        }
        ...
        let settings_config = match serde_json::to_value(&config) {
            Ok(v) => v,
            Err(e) => {
                log::warn!("Failed to serialize Hermes provider '{id}': {e}");
                continue;
            }
        };
        ...
        if let Err(e) = state.db.save_provider("hermes", &provider) {
            log::warn!("Failed to import Hermes provider '{id}': {e}");
            continue;
        }
...
///
/// This removes a specific provider from ~/.hermes/hermes.json
/// without affecting other providers in the file.
