...
/// Strip trailing `\n[message_id: ...]` metadata injected by Hermes gateway.
...
pub fn scan_sessions() -> Vec<SessionMeta> {
    let agents_dir = get_hermes_dir().join("agents");
...
pub fn load_messages(path: &Path) -> Result<Vec<SessionMessage>, String> {
    ...
    for line in reader.lines() {
        ...
        // Map Hermes roles to our standard roles
...
fn parse_session(
    ...
    Some(SessionMeta {
        ...
        resume_command: None, // Hermes sessions are gateway-managed, no CLI resume
    })
}
...
fn prune_sessions_index(
    ...
    let content = std::fs::read_to_string(index_path).map_err(|e| {
        format!(
            "Failed to read Hermes sessions index {}: {e}",
            index_path.display()
        )
    })?;
    let mut index: serde_json::Map<String, Value> =
        serde_json::from_str(&content).map_err(|e| {
            format!(
                "Failed to parse Hermes sessions index {}: {e}",
                index_path.display()
            )
        })?;
    ...
    write_json_file(index_path, &index).map_err(|e| {
        format!(
            "Failed to update Hermes sessions index {}: {e}",
            index_path.display()
        )
    })
}
