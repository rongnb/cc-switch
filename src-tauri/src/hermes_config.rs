//! Hermes 配置文件读写模块
//!
//! 处理 `~/.hermes/hermes.json` 配置文件的读写操作（JSON5 格式）。
...
const HERMES_DEFAULT_SOURCE: &str =
    "{\n  models: {\n    mode: 'merge',\n    providers: {},\n  },\n}\n";
const HERMES_TOOLS_PROFILES: &[&str] = &["minimal", "coding", "messaging", "full"];
...
/// 获取 Hermes 配置文件路径
///
/// 返回 `~/.hermes/hermes.json`
pub fn get_hermes_config_path() -> PathBuf {
    get_hermes_dir().join("hermes.json")
}

fn default_hermes_config_value() -> Value {
    json!({
        "models": {
            "mode": "merge",
            "providers": {}
...
/// Hermes 供应商配置（对应 models.providers 中的条目）
...
pub struct HermesProviderConfig {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub models: Vec<HermesModelEntry>,
...
pub struct HermesModelEntry {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub cost: Option<HermesModelCost>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// Hermes 默认模型配置（agents.defaults.model）
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HermesDefaultModel {
    pub primary: String,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
...
/// Hermes 模型目录条目（agents.defaults.models 中的值）
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HermesModelCatalogEntry {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub alias: Option<String>,
    #[serde(flatten)]
    pub extra: HashMap<String, Value>,
...
/// Hermes agents.defaults 配置
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HermesAgentsDefaults {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub model: Option<HermesDefaultModel>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models: Option<HashMap<String, HermesModelCatalogEntry>>,
    #[serde(flatten)]
    pub extra: HashMap<String, Value>,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[allow(dead_code)]
pub struct HermesAgents {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub defaults: Option<HermesAgentsDefaults>,
    #[serde(flatten)]
    pub extra: HashMap<String, Value>,
}

/// Hermes env 配置（hermes.json 的 env 节点）
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Hermes tools 配置（hermes.json 的 tools 节点）
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct HermesToolsConfig {
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub fn read_hermes_config() -> Result<Value, AppError> {
    ...
    if !path.exists() {
        return Ok(default_hermes_config_value());
    }

    let content = fs::read_to_string(&path).map_err(|e| AppError::io(&path, e))?;
    json5::from_str(&content)
        .map_err(|e| AppError::Config(format!("Failed to parse Hermes config as JSON5: {e}")))
}
...
pub fn scan_hermes_config_health() -> Result<Vec<HermesHealthWarning>, AppError> {
    ...
    match json5::from_str::<Value>(&content) {
        Ok(config) => Ok(scan_hermes_health_from_value(&config)),
        Err(err) => Ok(vec![HermesHealthWarning {
            code: "config_parse_failed".to_string(),
            message: format!("Hermes config could not be parsed as JSON5: {err}"),
            path: Some(path.display().to_string()),
        }]),
    }
}

struct HermesConfigDocument {
    path: PathBuf,
    original_source: Option<String>,
    text: RtJSONText,
}

impl HermesConfigDocument {
    fn load() -> Result<Self, AppError> {
        ...
        let source = original_source
            .clone()
            .unwrap_or_else(|| HERMES_DEFAULT_SOURCE.to_string());
        let text = rt_from_str(&source).map_err(|e| {
            AppError::Config(format!(
                "Failed to parse Hermes config as round-trip JSON5 document: {}",
                e.message
            ))
        })?;
    ...
    fn set_root_section(&mut self, key: &str, value: &Value) -> Result<(), AppError> {
        let RtJSONValue::JSONObject {
            ...
            return Err(AppError::Config(
                "Hermes config root must be a JSON5 object".to_string(),
            ));
        };
    ...
    fn save(self) -> Result<HermesWriteOutcome, AppError> {
        ...
        if current_source != self.original_source {
            return Err(AppError::Config(
                "Hermes config changed on disk. Please reload and try again.".to_string(),
            ));
        }

        let next_source = self.text.to_string();
        if current_source.as_deref() == Some(next_source.as_str()) {
            let warnings = scan_hermes_health_from_value(
                &json5::from_str::<Value>(&next_source).map_err(|e| {
                    AppError::Config(format!(
                        "Failed to parse unchanged Hermes config as JSON5: {e}"
                    ))
                })?,
            );

            return Ok(HermesWriteOutcome {
                backup_path: None,
                warnings,
            });
        }
        ...
        let backup_path = current_source
            .as_ref()
            .map(|source| create_hermes_backup(source))
            .transpose()?
        ...
        let warnings = scan_hermes_health_from_value(
            &json5::from_str::<Value>(&next_source).map_err(|e| {
                AppError::Config(format!(
                    "Failed to parse newly written Hermes config as JSON5: {e}"
                ))
            })?,
        );

        log::debug!("Hermes config written to {:?}", self.path);
...
fn write_root_section(section: &str, value: &Value) -> Result<HermesWriteOutcome, AppError> {
    let mut document = HermesConfigDocument::load()?;
...
fn warning(code: &str, message: impl Into<String>, path: Option<&str>) -> HermesHealthWarning {
    HermesHealthWarning {
        code: code.to_string(),
...
fn scan_hermes_health_from_value(config: &Value) -> Vec<HermesHealthWarning> {
    ...
    {
        if !HERMES_TOOLS_PROFILES.contains(&profile) {
...
pub fn set_provider(id: &str, provider_config: Value) -> Result<HermesWriteOutcome, AppError> {
    let mut full_config = read_hermes_config()?;
...
/// 删除供应商配置
pub fn remove_provider(id: &str) -> Result<HermesWriteOutcome, AppError> {
...
/// 获取所有供应商配置（类型化）
pub fn get_typed_providers() -> Result<IndexMap<String, HermesProviderConfig>, AppError> {
    let providers = get_providers()?;
    let mut result = IndexMap::new();

    for (id, value) in providers {
        match serde_json::from_value::<HermesProviderConfig>(value.clone()) {
            Ok(config) => {
                result.insert(id, config);
            }
            Err(e) => {
                log::warn!("Failed to parse Hermes provider '{id}': {e}");
            }
        }
    }
...
pub fn get_default_model() -> Result<Option<HermesDefaultModel>, AppError> {
    let config = read_hermes_config()?;
...
pub fn set_default_model(model: &HermesDefaultModel) -> Result<HermesWriteOutcome, AppError> {
    let mut config = read_hermes_config()?;
...
pub fn get_model_catalog() -> Result<Option<HashMap<String, HermesModelCatalogEntry>>, AppError> {
    let config = read_hermes_config()?;
...
pub fn set_model_catalog(
    catalog: &HashMap<String, HermesModelCatalogEntry>,
) -> Result<HermesWriteOutcome, AppError> {
    let mut config = read_hermes_config()?;
    let root = ensure_object(&mut config);
    let agents = root
...
pub fn get_agents_defaults() -> Result<Option<HermesAgentsDefaults>, AppError> {
    let config = read_hermes_config()?;
...
pub fn get_env_config() -> Result<HermesEnvConfig, AppError> {
    let config = read_hermes_config()?;
...
pub fn set_env_config(env: &HermesEnvConfig) -> Result<HermesWriteOutcome, AppError> {
...
pub fn get_tools_config() -> Result<HermesToolsConfig, AppError> {
    let config = read_hermes_config()?;

    let Some(tools_value) = config.get("tools") else {
        return Ok(HermesToolsConfig {
            profile: None,
            allow: Vec::new(),
            deny: Vec::new(),
            extra: HashMap::new(),
...
pub fn set_tools_config(tools: &HermesToolsConfig) -> Result<HermesWriteOutcome, AppError> {
...
mod tests {
    ...
    fn with_test_paths<T>(source: &str, test: impl FnOnce(&Path) -> T) -> T {
        ...
        let config_path = hermes_dir.join("hermes.json");
    ...
    #[test]
    fn scan_health_detects_known_hermes_issues() {
        let config = json!({
        ...
        let warnings = scan_hermes_health_from_value(&config);
        let codes = warnings
            .into_iter()
...
"#;
        ...
        with_test_paths(source, |_| {
            let outcome = set_default_model(&HermesDefaultModel {
            ...
            let written = fs::read_to_string(get_hermes_config_path()).unwrap();
...
"#;
        ...
        with_test_paths(source, |_| {
            ...
            let first_written = fs::read_to_string(get_hermes_config_path()).unwrap();
            let backup_dir = get_app_config_dir().join("backups").join("hermes");
            ...
            let second_written = fs::read_to_string(get_hermes_config_path()).unwrap();
...
"#;
        ...
        with_test_paths(source, |config_path| {
            let mut document = HermesConfigDocument::load().unwrap();
            document
            ...
            assert!(err.to_string().contains("Hermes config changed on disk"));
        });
    }
