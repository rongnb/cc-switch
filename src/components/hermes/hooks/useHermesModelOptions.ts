...
import type { HermesProviderConfig } from "@/types";
...
export function useHermesModelOptions(): {
  options: ModelOption[];
  isLoading: boolean;
} {
  const { data: providersData, isLoading } = useProvidersQuery("hermes");
  ...
  const options = useMemo<ModelOption[]>(() => {
    ...
    for (const [providerKey, provider] of Object.entries(allProviders)) {
      let config: HermesProviderConfig;
      try {
        config =
          typeof provider.settingsConfig === "string"
            ? (JSON.parse(provider.settingsConfig) as HermesProviderConfig)
            : (provider.settingsConfig as HermesProviderConfig);
      } catch {
        continue;
      }
