...
import {
  useHermesAgentsDefaults,
  useSaveHermesAgentsDefaults,
...
import type { HermesAgentsDefaults } from "@/types";
import { useHermesModelOptions } from "./hooks/useHermesModelOptions";
import { getHermesTimeoutInputValue } from "./utils";

const UNSET_SENTINEL = "__unset__";

const AgentsDefaultsPanel: React.FC = () => {
  const { t } = useTranslation();
  const { data: agentsData, isLoading } = useHermesAgentsDefaults();
  const saveAgentsMutation = useSaveHermesAgentsDefaults();
  const { options: modelOptions, isLoading: modelsLoading } =
    useHermesModelOptions();

  const [defaults, setDefaults] = useState<HermesAgentsDefaults | null>(null);
  ...
  useEffect(() => {
    ...
    if (agentsData) {
      ...
      setTimeout_(getHermesTimeoutInputValue(agentsData));
  ...
  const primaryOptions = useMemo(() => {
    ...
    if (
      ...
      result.unshift({
        value: primaryModel,
        label: t("hermes.agents.notInList", {
          value: primaryModel,
  ...
  const handleSave = async () => {
    try {
      ...
      const updated: HermesAgentsDefaults = { ...defaults };
      ...
      toast.success(t("hermes.agents.saveSuccess"));
    } catch (error) {
      const detail = extractErrorMessage(error);
      toast.error(t("hermes.agents.saveFailed"), {
    ...
    <div className="px-6 pt-4 pb-8">
      ...
      {hasLegacyTimeout && (
        <Alert className="mb-4 border-amber-500/30 bg-amber-500/5">
          ...
          <AlertTitle>
            {t("hermes.agents.legacyTimeoutTitle", {
              defaultValue: "Legacy timeout detected",
            })}
          </AlertTitle>
          <AlertDescription>
            {t("hermes.agents.legacyTimeoutDescription", {
              defaultValue:
        ...
        <div className="space-y-4">
          ...
          <div>
            <Label className="mb-1.5 block">
              {t("hermes.agents.primaryModel")}
            </Label>
            {noModels ? (
              <p className="text-xs text-muted-foreground italic">
                {t("hermes.agents.noModels", {
                  defaultValue:
                    "No configured provider models. Please add an Hermes provider first.",
                })}
              </p>
            ) : (
              <Select
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {t("hermes.agents.primaryModelHint")}
            </p>
          </div>
          ...
          <div>
            <Label className="mb-1.5 block">
              {t("hermes.agents.fallbackModels")}
            </Label>

            {fallbacks.length === 0 && !noModels && (
              <p className="text-xs text-muted-foreground italic mb-2">
                {t("hermes.agents.fallbackModelsHint")}
              </p>
            )}
      ...
      <div className="rounded-xl border border-border bg-card p-5 mb-4">
        <h3 className="text-sm font-medium mb-4">
          {t("hermes.agents.runtimeSection")}
        </h3>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <Label className="mb-1.5 block">
              {t("hermes.agents.workspace")}
            </Label>
            <Input
              value={workspace}
